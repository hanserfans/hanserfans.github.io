...
export default function ArticleGenerator() {
  ...
  // 注册全局状态更新函数和事件监听器
  useEffect(() => {
    console.log('🔧 ArticleGenerator组件挂载')
    console.log('🔍 Socket连接状态:', socket.connected)
    console.log('🆔 Socket ID:', socket.id)
    console.log('📡 Socket传输方式:', socket.io.engine?.transport?.name)

    // 设置全局状态更新函数
    console.log('📝 设置全局状态更新函数')
    globalUpdateFunctions.setCurrentStatus = setCurrentStatus
    globalUpdateFunctions.setProgress = setProgress
    globalUpdateFunctions.setGenerationProgress = setGenerationProgress
    globalUpdateFunctions.setIsGenerating = setIsGenerating

    // 注册全局事件监听器（如果尚未注册）
    console.log('🔗 注册全局WebSocket事件监听器')
    registerGlobalEventListeners()

    // 手动检查事件监听器是否已注册
    console.log('🔍 检查已注册的事件监听器:')
    ...
    return () => {
      ...
      globalUpdateFunctions.setIsGenerating = null
    }
  }, [])
